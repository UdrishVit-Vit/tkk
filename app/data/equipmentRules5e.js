...
const EQUIPMENT_GROUP_DATA_5E = [
  ...
  },
  {
    ...
    items: [
      ...
      ['tattooist-tools', 'Инструменты татуировщика', "Tattooist's Tools", 'TL', 'tool', 'Инструменты татуировщика нужны, чтобы наносить, править и читать татуировки — от клановой метки до ритуальных узоров народов Эноа.', {
        sourceUrl: '/dnd5e/equipment?e=tattooist-tools',
        quick: [
          { label: 'Раздел', value: 'Обычные инструменты' },
          { label: 'Характеристика', value: 'Ловкость' },
          { label: 'Стоимость', value: '25 зм, вес 1,5 кг' }
        ],
        terms: ['инструменты', 'татуировки', 'Эноа', 'аджаиды', 'вету'],
        blocks: [
          {
            title: 'Что входит в набор',
            paragraphs: [
              'Иглы и костяные резцы, малое зеркало, каменные чаши, растительные и минеральные пигменты, жгуты, повязки и заживляющая мазь.',
              'Чернила расходуются: одного набора хватает примерно на десять татуировок, новый запас пигментов стоит 5 зм. Ритуальные татуировки требуют собственных составов, перечисленных в описании соответствующей черты.'
            ]
          },
          {
            title: 'Нанесение татуировки',
            paragraphs: [
              'Простая татуировка занимает 1 час работы и проверку Ловкости (инструменты татуировщика) со Сл 10. Крупная и сложная — 8 часов и Сл 15.',
              'При провале рисунок выходит кривым: его можно свести или перекрыть, но следующая попытка на том же месте совершается с помехой. Проверка Мудрости (Медицина) со Сл 10 в первые сутки снимает воспаление; без ухода первая ночь после большой татуировки не считается продолжительным отдыхом.'
            ]
          },
          {
            title: 'Чтение татуировок',
            paragraphs: [
              'Владение набором позволяет добавить бонус мастерства к проверке Интеллекта (История), когда вы разбираете чужую татуировку.'
            ],
            bullets: [
              'Сл 10 — народ и родина носителя.',
              'Сл 15 — клан, каста или ремесло.',
              'Сл 20 — принесённая клятва, долг или наказание.',
              'Сл 25 — рука мастера и место, где работа была сделана.',
              'Проверка Интеллекта (Расследование) со Сл 15 показывает, что татуировку переделали, перекрыли или подделали; для работы хорошего мастера — Сл 20.'
            ]
          },
          {
            title: 'Кому нужен набор',
            paragraphs: [
              'Черты, которые прямо требуют этих инструментов, наносятся по собственным правилам, а не по общим.'
            ],
            bullets: [
              '[Аджаиды](/dnd5e/races/adzhaidy) — «Татуировки Ветров» на крови трёхногого додора; аджаидские мастера чернил составляют отдельную касту.',
              '[Вету](/dnd5e/races/vetu) — янты, ритуальные узоры дома, долга и пережитой боли. Игровых свойств они не дают, но наносятся тем же набором.',
              '[Джабари](/dnd5e/races/jabari) — племенная роспись и клейма почтения духам.'
            ]
          }
        ]
      }]
