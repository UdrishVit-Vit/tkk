...
const contentByCat = Object.fromEntries(PANTHEON_CONTENT.categories.map(cat => [cat.slug, cat]))
...
function scrollToCat(index, filter = false) {
  if (filter) selectedCat.value = index === null ? 'all' : PANTHEON_THREAD.sections[index].slug
...
function handleKeyboard(event) {
  ...
  if (event.key === 'PageDown') {
    ...
    scrollToCat(Math.min(PANTHEON_THREAD.sections.length - 1, activeIndex.value + 1))
  }
  if (event.key === 'PageUp') {
    event.preventDefault()
  ...
  <main class="lore-history" :style="{ background: theme.bg }">
    ...
    <section ref="scroller" class="lore-viewport" @scroll="handleScroll">
      ...
      <div class="history-canvas">
        ...
        <section ref="introSection" class="history-intro timeline-row" aria-labelledby="pantheon-title">
          <div class="history-intro__copy">
            <p>{{ PANTHEON_THREAD.eyebrow }}</p>
            <h1 id="pantheon-title">{{ PANTHEON_THREAD.title }}</h1>
            <aside>
              <span>Архив Башни</span>
              <p>{{ PANTHEON_THREAD.archiveNote }}</p>
            </aside>
          </div>
          <button
            type="button"
            ...
            @click="$emit('up')"
          >
            <img src="/assets/nodes/pantheon-lore.webp" width="256" height="256" alt="" decoding="async">
          </button>
        </section>
        ...
        >
          <div class="history-era__heading timeline-row">
            <NuxtLink :to="`/lore/pantheon/${section.slug}`" class="history-era__title">
              <h2>{{ section.title }}</h2>
              <p>{{ section.summary }}</p>
            </NuxtLink>
            <NuxtLink
              :to="`/lore/pantheon/${section.slug}`"
          ...
          <div v-if="contentByCat[section.slug]?.chapters.length" class="history-era__chapters">
            ...
            >
              <NuxtLink
                ...
                :to="`/lore/pantheon/${section.slug}?chapter=${chapter.slug}`"
              >
        ...
        <div class="history-end timeline-row" aria-hidden="true">
          <i /><span>Пантеон пополняется</span>
        </div>
      </div>
    </section>
  </main>
