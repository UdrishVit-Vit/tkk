...
function statesForCat(catIndex) {
  const section = PANTHEON_THREAD.sections[catIndex]
  const content = PANTHEON_CONTENT.categories?.[catIndex]
  if (!section || !content) return []

  const states = []
  if (content.intro?.length || !content.chapters?.length) {
    states.push({ catIndex, stateIndex: 0, title: section.title, slug: '', path: `/lore/pantheon/${section.slug}` })
  }
  for (const chapter of content.chapters || []) {
    states.push({
      ...
      title: chapter.title,
      slug: chapter.slug,
      path: `/lore/pantheon/${section.slug}`,
    })
  }
  return states
}
  ...
  <main class="era-page">
    ...
    <section class="era-heading">
      <NuxtLink
        ...
        aria-label="Вернуться к пантеону"
      >
    ...
    <nav v-if="chapters.length" class="era-thread" aria-label="Божества и сущности рода">
