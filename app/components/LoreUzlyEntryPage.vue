...
import { LORE_GLOSSARY_BY_ID, LORE_ENTITY_TYPES } from '~/data/loreGlossary.js'
...
const groupedChapters = computed(() => {
  ...
  chapters.value.forEach((chapter, chapterIndex) => {
    const season = chapter.season || ''
    const last = groups.at(-1)
    if (last && last.season === season) last.items.push({ chapter, chapterIndex })
    else groups.push({ season, items: [{ chapter, chapterIndex }] })
  })
  return groups
})
const seasons = computed(() => groupedChapters.value.map(group => group.season).filter(Boolean))
const selectedSeason = ref('')
const visibleItems = computed(() => {
  const group = groupedChapters.value.find(item => item.season === selectedSeason.value)
  return group ? group.items : groupedChapters.value.flatMap(item => item.items)
})
// Всплывающая карточка термина — та же, что в глоссарии. Имя в тексте книги
// ведёт не сразу в статью: читатель чаще хочет вспомнить, кто такой Драгмир,
// и вернуться к строке, а не уходить со страницы.
...
function openTerm(id) {
  closeTermCard()
  router.push(`/lore/glossary/${id}`)
}

// Ссылкой становится первое упоминание имени в сцене: дальше по сцене читатель
// уже знает, кто это, а подчёркнутые «Саргон» в каждой строке — рябь. Счёт
// сбрасывается на каждом подзаголовке.
function withFirstMentions(blocks) {
  let seen = new Set()
  return (blocks || []).map((block) => {
    ...
    const paragraphs = block.paragraphs.map((text) => {
      const allow = new Set()
      for (const token of tokenizeLoreText(text)) {
        if (token.type !== 'term') continue
        if (token.ids.some(id => seen.has(id))) continue
        token.ids.forEach(id => seen.add(id))
        token.ids.forEach(id => allow.add(id))
      }
      return { text, allow }
    })
...
const activeBlocks = computed(() => withFirstMentions(
  activeChapter.value?.blocks || props.content?.intro || [],
))
...
function statesForCat(catIndex) {
  const section = UZLY_THREAD.sections[catIndex]
  const content = UZLY_CONTENT.categories?.[catIndex]
  if (!section || !content) return []

  const states = []
  if (content.intro?.length || !content.chapters?.length) {
    states.push({ catIndex, stateIndex: 0, title: section.title, slug: '', path: `/lore/uzly/${section.slug}` })
  }
...
watch([activeChapter, seasons], () => {
  if (activeChapter.value?.season) selectedSeason.value = activeChapter.value.season
...
<template>
  <main class="era-page">
    ...
    <div v-if="seasons.length > 1" class="era-seasons" role="tablist" aria-label="Сезоны">
      <button
        v-for="s in seasons"
        :key="s"
        type="button"
        ...
        :class="{ 'is-active': selectedSeason === s }"
        @click="selectedSeason = s"
      >{{ s }}</button>
    </div>

    <nav v-if="chapters.length" class="era-thread" aria-label="Серии сезона">
      <div class="era-thread__line" aria-hidden="true"><i /><i /><i /></div>
      <button
        v-for="item in visibleItems"
        :key="item.chapter.slug"
        type="button"
        :class="{ 'is-active': activeSlug === item.chapter.slug }"
        @click="selectChapter(item.chapter.slug)"
      >
        <span>{{ index + 1 }}.{{ item.chapterIndex + 1 }}</span>
        ...
        <Transition :name="transitionName" mode="out-in">
          <div :key="sceneKey" class="era-reader__scene">
            <header v-if="activeChapter">
              <span v-if="activeChapter.season">{{ activeChapter.season }} · {{ index + 1 }}.{{ activePosition }}</span>
              <span v-else>{{ index + 1 }}.{{ activePosition }}</span>
              <h2>{{ activeTitle }}</h2>
              <p v-if="activeChapter.summary" class="era-reader__lead">{{ activeChapter.summary }}</p>
            </header>

            <div class="era-reader__body">
              <template v-for="(block, blockIndex) in activeBlocks" :key="`${activeSlug}:${blockIndex}`">
                <dl v-if="block.type === 'facts'" class="era-facts">
                  <div v-for="item in block.items" :key="item.label">
                    <dt>{{ item.label }}</dt>
                    <dd>{{ item.value }}</dd>
                  </div>
                </dl>
                <h3 v-else-if="block.type === 'heading'" class="era-scene">{{ block.text }}</h3>
                <div v-else-if="block.type === 'divider'" class="era-break" aria-hidden="true"><i /></div>
                <blockquote v-else-if="block.type === 'verse'">
                  <i aria-hidden="true" />
                  <p v-for="line in block.paragraphs" :key="line.text"><LoreRichText :text="line.text" :allow="line.allow" @preview="showTermCard" /></p>
                </blockquote>
                <template v-else>
                  <p v-for="paragraph in block.paragraphs" :key="paragraph.text"><LoreRichText :text="paragraph.text" :allow="paragraph.allow" @preview="showTermCard" /></p>
                </template>
              </template>
              <p v-if="!activeBlocks.length" class="era-reader__empty">Текст этого узла ещё не внесён в архив.</p>
            </div>
          </div>
        </Transition>
...
.era-reader{position:relative;min-height:380px;padding:45px 42px 29px;border-inline:1px solid rgba(var(--theme-accent-rgb),.12);background:linear-gradient(180deg,rgba(var(--theme-surface-rgb),.72),rgba(var(--theme-surface-rgb),.34));box-shadow:0 28px 65px rgba(0,0,0,.16);transition:opacity .18s,transform .18s}.era-reader::before,.era-reader::after{content:'';position:absolute;width:24px;height:24px;border-color:rgba(var(--theme-accent-rgb),.42)}.era-reader::before{top:16px;left:16px;border-top:1px solid;border-left:1px solid}.era-reader::after{right:16px;bottom:16px;border-right:1px solid;border-bottom:1px solid}.era-reader__scene>header{position:relative;padding:0 0 31px;text-align:center;border-bottom:1px solid rgba(var(--theme-accent-rgb),.14)}.era-reader__scene>header::after{content:'';position:absolute;bottom:-1px;left:50%;width:78px;height:1px;transform:translateX(-50%);background:rgba(var(--theme-accent-strong-rgb),.68)}.era-reader__scene>header>span{font:500 7px/1 'Hanken Grotesk',sans-serif;color:rgba(var(--theme-accent-rgb),.58)}.era-reader h2{margin:12px 0 0;font:600 clamp(36px,5vw,54px)/.96 'Cormorant Garamond',serif;color:rgba(var(--theme-heading-rgb),.96);text-wrap:balance}.era-reader__body{padding:34px 0 10px}.era-reader__body .lore-term{border-bottom-color:rgba(var(--theme-accent-rgb),.26)}.era-reader__body>p{margin:0 0 1.35em;font:400 20px/1.66 'Cormorant Garamond',serif;color:rgba(var(--theme-text-rgb),.78);text-wrap:pretty}.era-reader__body>p:first-child::first-letter{float:left;margin:.08em .13em 0 0;font:600 4em/.72 'Cormorant Garamond',serif;color:rgba(var(--theme-accent-strong-rgb),.88)}.era-reader blockquote{position:relative;margin:30px 0;padding:31px 42px;border-block:1px solid rgba(var(--theme-accent-rgb),.16);background:linear-gradient(90deg,transparent,rgba(var(--theme-accent-rgb),.032),transparent);text-align:center}.era-reader blockquote>i{position:absolute;top:-6px;left:50%;width:11px;height:11px;border:1px solid rgba(var(--theme-accent-rgb),.42);background:rgb(var(--theme-surface-rgb));transform:translateX(-50%) rotate(45deg)}.era-reader blockquote p{margin:0;font:italic 18px/1.55 'Cormorant Garamond',serif;color:rgba(var(--theme-heading-rgb),.74)}.era-reader__lead{position:relative;margin:26px auto 0;max-width:62ch;font:italic 20px/1.6 'Cormorant Garamond',serif;color:rgba(var(--theme-text-rgb),.8);text-wrap:pretty}.era-reader__lead::before{content:'';position:absolute;top:-14px;left:50%;width:46px;height:1px;transform:translateX(-50%);background:linear-gradient(90deg,transparent,rgba(var(--theme-accent-rgb),.5),transparent)}.era-break{display:flex;align-items:center;justify-content:center;margin:34px 0 32px}.era-break::before,.era-break::after{content:'';height:1px;width:min(21%,150px);background:linear-gradient(90deg,transparent,rgba(var(--theme-accent-rgb),.34))}.era-break::after{background:linear-gradient(90deg,rgba(var(--theme-accent-rgb),.34),transparent)}.era-break i{width:7px;height:7px;margin:0 15px;border:1px solid rgba(var(--theme-accent-strong-rgb),.6);transform:rotate(45deg)}.era-reader__body>.era-break+p::first-letter{margin:0;font:inherit;color:inherit;float:none}.era-scene{position:relative;margin:38px 0 20px;padding-top:20px;font:600 24px/1.14 'Cormorant Garamond',serif;color:rgba(var(--theme-heading-rgb),.9);text-wrap:balance}.era-scene::before{content:'';position:absolute;top:0;left:0;width:64px;height:1px;background:linear-gradient(90deg,rgba(var(--theme-accent-strong-rgb),.62),transparent)}.era-reader__body>.era-scene:first-child{margin-top:0}.era-reader__body>.era-scene:first-child+p{margin-top:2em}.era-reader__body>.era-scene:first-child+p::first-letter{margin:0 .06em 0 0;font:600 2.8em/0 'Cormorant Garamond',serif;color:rgba(var(--theme-accent-strong-rgb),.72)}.era-reader__empty{text-align:center;font-style:italic;color:rgba(var(--theme-text-rgb),.42)!important}.era-facts{display:flex;margin:0 0 28px;flex-wrap:wrap;gap:10px 14px;padding:18px 22px;border:1px solid rgba(var(--theme-accent-rgb),.22);background:linear-gradient(90deg,transparent,rgba(var(--theme-accent-rgb),.035),transparent)}.era-facts>div{min-width:180px;flex:1 1 auto}.era-facts dt{margin:0 0 4px;font:600 7px/1 'Hanken Grotesk',sans-serif;letter-spacing:.16em;text-transform:uppercase;color:rgba(var(--theme-accent-rgb),.62)}.era-facts dd{margin:0;font:500 15px/1.36 'Cormorant Garamond',serif;color:rgba(var(--theme-heading-rgb),.88)}.era-reader__progress{display:flex;margin-top:25px;align-items:center;justify-content:center;gap:13px;color:rgba(var(--theme-text-rgb),.33);font:500 6px/1 'Hanken Grotesk',sans-serif;letter-spacing:.15em}.era-reader__progress i{width:32px;height:1px;background:linear-gradient(90deg,transparent,rgba(var(--theme-accent-rgb),.38))}.era-reader__progress i:last-child{transform:scaleX(-1)}
