...
function statesForCat(catIndex) {
  const section = FACTIONS_THREAD.sections[catIndex]
  const content = FACTIONS_CONTENT.categories?.[catIndex]
  if (!section || !content) return []

  const states = []
  if (content.intro?.length || !content.chapters?.length) {
    states.push({ catIndex, stateIndex: 0, title: section.title, slug: '', path: `/lore/factions/${section.slug}` })
  }
  for (const chapter of content.chapters || []) {
    states.push({
      ...
      title: chapter.title,
      slug: chapter.slug,
      path: `/lore/factions/${section.slug}`,
    })
  }
  return states
}
  ...
  <main class="era-page">
    ...
    <section class="era-heading">
      <NuxtLink
        ...
        aria-label="Вернуться к фракциям"
      >
    ...
    <nav v-if="chapters.length" class="era-thread" aria-label="Главы фракции">
          ...
          <div :key="sceneKey" class="era-reader__scene">
            ...
            <div class="era-reader__body">
              <template v-for="(block, blockIndex) in activeBlocks" :key="`${activeSlug}:${blockIndex}`">
                ...
                <ul v-else-if="block.type === 'list'" class="era-list">
              ...
              <p v-if="!activeBlocks.length" class="era-reader__empty">Текст этого раздела ещё не внесён в архив.</p>
            </div>
          </div>
        </Transition>
...
.era-facts{display:flex;margin:0 0 28px;flex-wrap:wrap;gap:10px 14px;padding:18px 22px;border:1px solid rgba(var(--theme-accent-rgb),.22);background:linear-gradient(90deg,transparent,rgba(var(--theme-accent-rgb),.035),transparent)}.era-facts>div{min-width:180px;flex:1 1 auto}.era-facts dt{margin:0 0 4px;font:600 7px/1 'Hanken Grotesk',sans-serif;letter-spacing:.16em;text-transform:uppercase;color:rgba(var(--theme-accent-rgb),.62)}.era-facts dd{margin:0;font:500 15px/1.36 'Cormorant Garamond',serif;color:rgba(var(--theme-heading-rgb),.88)}
.era-list{margin:0 0 1.35em;padding:0;list-style:none}.era-list li{position:relative;margin:0 0 .9em;padding-left:24px;font:400 20px/1.5 'Cormorant Garamond',serif;color:rgba(var(--theme-text-rgb),.78)}.era-list li::before{content:'';position:absolute;top:.55em;left:2px;width:7px;height:7px;border:1px solid rgba(var(--theme-accent-rgb),.55);background:rgb(var(--theme-surface-rgb));transform:rotate(45deg)}
