...
const contentByCat = Object.fromEntries(UZLY_CONTENT.categories.map(cat => [cat.slug, cat]))
...
function chapterPreview(chapter) {
  const text = (chapter.blocks || [])
    .flatMap(block => block.paragraphs || (block.items || []).map(item => `${item.label}: ${item.value}`))
    .join(' ')
    .trim()
...
function scrollToCat(index, filter = false) {
  if (filter) selectedCat.value = index === null ? 'all' : UZLY_THREAD.sections[index].slug
...
function handleKeyboard(event) {
  ...
  if (event.key === 'PageDown') {
    ...
    scrollToCat(Math.min(UZLY_THREAD.sections.length - 1, activeIndex.value + 1))
  }
  if (event.key === 'PageUp') {
    event.preventDefault()
  ...
  <main class="lore-history" :style="{ background: theme.bg }">
    ...
    <section ref="scroller" class="lore-viewport" @scroll="handleScroll">
      ...
      <div class="history-canvas">
        ...
        <section ref="introSection" class="history-intro timeline-row" aria-labelledby="uzly-title">
          <div class="history-intro__copy">
            <p>{{ UZLY_THREAD.eyebrow }}</p>
            <h1 id="uzly-title">{{ UZLY_THREAD.title }}</h1>
            <aside>
              <span>Архив Башни</span>
              <p>{{ UZLY_THREAD.archiveNote }}</p>
            </aside>
          </div>
          <button
            type="button"
            ...
            @click="$emit('up')"
          >
            <img src="/assets/nodes/uzly-lore.webp" width="256" height="256" alt="" decoding="async">
          </button>
        </section>
        ...
        >
          <div class="history-era__heading timeline-row">
            <NuxtLink :to="`/lore/uzly/${section.slug}`" class="history-era__title">
              <h2>{{ section.title }}</h2>
              <p>{{ section.summary }}</p>
            </NuxtLink>
            <NuxtLink
              :to="`/lore/uzly/${section.slug}`"
          ...
          <p v-if="contentByCat[section.slug]?.chapters.length" class="history-era__count">
            {{ contentByCat[section.slug].chapters.length }} серий — открыть узел, чтобы читать
          </p>
        </section>

        <div class="history-end timeline-row" aria-hidden="true">
          <i /><span>Ещё не все узлы распутаны</span>
        </div>
      </div>
    </section>
  </main>
...
.history-era__count{max-width:400px;margin:8px 0 0 auto;text-align:right;font:italic 14px/1.4 'Cormorant Garamond',serif;color:rgba(var(--theme-accent-rgb),.5)}
...
@media(max-width:1000px){.lore-rail{display:none}.lore-viewport{left:0}.lore-history__texture{left:68px}.history-canvas{max-width:780px;padding-inline:24px}.timeline-row{grid-template-columns:0 64px minmax(0,1fr)}.history-thread{left:56px}.history-intro__copy,.history-era__title{grid-column:3;text-align:left}.history-intro aside{margin-left:0;margin-right:auto;padding-right:0;padding-left:18px;border-right:0;border-left:1px solid rgba(var(--theme-accent-rgb),.32)}.history-intro__sigil,.history-node--era{grid-column:2}.history-era__title p,.history-era__count{margin-left:0;margin-right:auto;text-align:left}.history-entry--left .history-entry__card,.history-entry--right .history-entry__card{grid-column:3;text-align:left}.history-entry__node{grid-column:2}.history-entry__node::before,.history-entry--left .history-entry__node::before,.history-entry--right .history-entry__node::before{left:50%;right:auto;transform:none}.history-entry--left small{right:12px;left:auto}.history-end i{grid-column:2}.history-end span{left:56px;transform:translateX(-50%)}}
