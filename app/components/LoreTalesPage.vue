...
const contentByCat = Object.fromEntries(TALES_CONTENT.categories.map(cat => [cat.slug, cat]))
...
function scrollToCat(index, filter = false) {
  if (filter) selectedCat.value = index === null ? 'all' : TALES_THREAD.sections[index].slug
...
function handleKeyboard(event) {
  ...
  if (event.key === 'PageDown') {
    ...
    scrollToCat(Math.min(TALES_THREAD.sections.length - 1, activeIndex.value + 1))
  }
  if (event.key === 'PageUp') {
    event.preventDefault()
  ...
  <main class="lore-history" :style="{ background: theme.bg }">
    ...
    <section ref="scroller" class="lore-viewport" @scroll="handleScroll">
      ...
      <div class="history-canvas">
        ...
        <section ref="introSection" class="history-intro timeline-row" aria-labelledby="tales-title">
          <div class="history-intro__copy">
            <p>{{ TALES_THREAD.eyebrow }}</p>
            <h1 id="tales-title">{{ TALES_THREAD.title }}</h1>
            <aside>
              <span>Архив Башни</span>
              <p>{{ TALES_THREAD.archiveNote }}</p>
            </aside>
          </div>
          <button
            type="button"
            ...
            @click="$emit('up')"
          >
            <img src="/assets/nodes/tales-lore.webp" width="256" height="256" alt="" decoding="async">
          </button>
        </section>
        ...
        >
          <div class="history-era__heading timeline-row">
            <NuxtLink :to="`/lore/tales/${section.slug}`" class="history-era__title">
              <h2>{{ section.title }}</h2>
              <p>{{ section.summary }}</p>
            </NuxtLink>
            <NuxtLink
          ...
          <div v-if="contentByCat[section.slug]?.chapters.length" class="history-era__chapters">
            ...
            >
              <NuxtLink
                ...
                :to="`/lore/tales/${section.slug}?chapter=${chapter.slug}`"
              >
        ...
        <div class="history-end timeline-row" aria-hidden="true">
          <i /><span>Караван-сарай хранит ещё немало историй</span>
        </div>
      </div>
    </section>
  </main>
