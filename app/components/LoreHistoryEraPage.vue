...
function statesForEra(eraIndex) {
  const section = HISTORY_THREAD.sections[eraIndex]
  const content = historyContent.eras?.[eraIndex]
  if (!section || !content) return []

  const states = []
  if (content.intro?.length || !content.chapters?.length) {
    states.push({ eraIndex, stateIndex: 0, title: section.title, slug: '', path: `/lore/history/${section.slug}` })
  }
  for (const chapter of content.chapters || []) {
    states.push({
      ...
      title: chapter.title,
      slug: chapter.slug,
      path: `/lore/history/${section.slug}`,
    })
  }
  return states
}

const currentStates = computed(() => statesForEra(props.index))
...
const previousTarget = computed(() => {
  ...
  const previousStates = statesForEra(props.index - 1)
...
const nextTarget = computed(() => {
  ...
  return statesForEra(props.index + 1)[0] || null
})
        ...
        <Transition :name="transitionName" mode="out-in">
          <div :key="sceneKey" class="era-reader__scene">
            ...
            <div class="era-reader__body">
              ...
              <p v-if="!activeBlocks.length" class="era-reader__empty">Текст этого раздела ещё не внесён в летопись.</p>
            </div>
          </div>
        </Transition>

        <footer class="era-reader__progress" aria-label="Положение в эпохе">
          <i />
...
.page-forward-enter-active,.page-forward-leave-active,.page-backward-enter-active,.page-backward-leave-active{transition:opacity .28s ease,transform .28s ease,filter .28s ease}.page-forward-enter-from{opacity:0;transform:translateX(42px);filter:blur(2px)}.page-forward-leave-to{opacity:0;transform:translateX(-42px);filter:blur(2px)}.page-backward-enter-from{opacity:0;transform:translateX(-42px);filter:blur(2px)}.page-backward-leave-to{opacity:0;transform:translateX(42px);filter:blur(2px)}
...
@media(max-width:680px){.era-page{padding-bottom:48px}.era-page__header{padding:19px 20px}.era-page__header>span{display:none}.era-heading{margin-top:43px;padding:0 24px;flex-direction:column;gap:27px;text-align:center}.era-heading__knot{width:50px;height:50px}.era-heading h1{font-size:45px}.era-thread{margin-top:42px;padding:28px 15px;gap:15px 11px}.era-thread button{width:calc(50% - 6px);min-height:104px;padding:20px 12px}.era-thread button>strong{font-size:16px}.era-stage{width:100%;margin-top:40px;padding:0 16px 17px;grid-template-columns:1fr 1fr;border-bottom:1px solid rgba(var(--theme-accent-rgb),.13)}.era-reader{grid-column:1/-1;grid-row:1;min-height:330px;padding:38px 22px 23px;border-inline:0;border-bottom:1px solid rgba(var(--theme-accent-rgb),.13)}.era-reader__body{padding-top:30px}.era-reader__body>p{font-size:19px;line-height:1.62}.era-reader blockquote{margin:26px -2px;padding:27px 14px}.era-reader blockquote p{font-size:17px}.era-direction,.era-direction-placeholder{position:relative;top:auto;grid-row:2;min-height:116px;padding:23px 9px 12px}.era-direction--previous{grid-column:1}.era-direction--next{grid-column:2}.era-direction small{margin:9px 0 6px}.era-direction strong{font-size:16px}.era-direction__arrow{height:18px}.era-direction__arrow b{width:19px;height:19px;font-size:18px}.era-stage__swipe{display:block;grid-column:1/-1;grid-row:3;margin:10px 0 0;text-align:center;font:500 6px/1 'Hanken Grotesk',sans-serif;letter-spacing:.18em;text-transform:uppercase;color:rgba(var(--theme-text-rgb),.27)}.era-stage__swipe span{padding:0 8px;color:rgba(var(--theme-accent-rgb),.55)}}
@media(prefers-reduced-motion:reduce){.era-reader,.era-thread button,.page-forward-enter-active,.page-forward-leave-active,.page-backward-enter-active,.page-backward-leave-active{transition:none!important}}
</style>
