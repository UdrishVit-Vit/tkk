...
function onLinkEnter(event, token) {
  ...
  showTimer = window.setTimeout(async () => {
    ...
    tip.value = { mode: 'rule', ...info, ...tipPosition(target) }
  }, 200)
}
...
<template>
  <span class="rt">
    <template v-for="(token, i) in tokens" :key="i">
      ...
      <span v-else-if="token.type === 'adv'" class="rt-adv">{{ token.text }}</span>
      <span v-else-if="token.type === 'dis'" class="rt-dis">{{ token.text }}</span>
