...
const state = reactive({
  ...
  theme: 'void', authed: false, bookmarks: [], query: '', cardQuery: '', infOpen: false, invOpen: false
})

// Allow deep-linking straight into a system map, e.g. /?system=5e (used by the
// races page's central node to jump back to "D&D 5e" instead of the home view).
const initialSystem = route.query.system
if (initialSystem && SYSTEMS.some(s => s.id === initialSystem)) {
  state.view = 'system'
  state.active = initialSystem
}

// Keep the address bar's ?system= in sync with whatever is actually on screen,
// so switching systems from inside the hub (theme/systems popover, knot clicks)
// never leaves a stale query string behind.
...
function selectSystem(id) { Object.assign(state, { view:'system', active:id, group:null, section:null, overlay:null }) }
...
const RACE_SECTION_ROUTES = { 'Расы и происхождения': '/dnd5e/races', 'Черты': '/dnd5e/feats', 'Предыстории': '/dnd5e/backgrounds', 'Заклинания': '/dnd5e/spells', 'Оружие': '/dnd5e/weapons', 'Доспехи': '/dnd5e/armor', 'Снаряжение': '/dnd5e/equipment', 'Бестиарий': '/dnd5e/bestiary' }
...
function openClass(name) { state.cls = state.cls === name ? null : name; state.infOpen = false; state.invOpen = false }
...
const vm = computed(() => {
  ...
  return {
    ...
    classFeatures: (cd.features||[]).map(f => ({ name:f[0], src:f[1], lvl:f[2], text:f[3],
      hasSpellTable: !!(f[4] && f[4].length), spellTable: (f[4]||[]).map(s=>({ lvl:s[0], spell:s[1] })) })),
  ...
  <div class="tkk" :style="{ background: vm.th.bg }">
    ...
    <div v-if="vm.isSystem" class="tkk-crumb">
      ...
      <template v-if="vm.showSectionCrumb">
        <span class="tkk-crumb-sep">/</span>
        <span class="tkk-crumb-link tkk-crumb-gold" @click="state.cls = null">{{ state.section }}</span>
      </template>
    ...
    <ClassPage v-if="vm.showClassPage" :vm="vm" :state="state" @up="state.cls = null" />
