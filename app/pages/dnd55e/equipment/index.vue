    ...
    <template #item="{ item }">
      <span class="equipment-list-card">
        <span class="equipment-glyph" :class="`kind-${item.raw.kind}`" aria-hidden="true">
          {{ item.raw.kind === 'weapon' ? '⚔' : item.raw.kind === 'armor' ? '◫' : item.raw.kind === 'tool' ? '◇' : item.raw.kind === 'mount' ? '♞' : item.raw.kind === 'vehicle' ? '⌁' : item.raw.kind === 'service' ? '¤' : '◆' }}
        </span>
...
.equipment-list-card{display:flex;align-items:center;gap:10px;width:100%;min-width:0}.equipment-glyph,.equipment-card-icon{display:grid;place-items:center;border:1px solid rgba(var(--theme-accent-rgb),.2);border-radius:8px;background:rgba(var(--theme-accent-rgb),.06);color:var(--theme-accent-strong)}.equipment-glyph{width:35px;height:35px;flex:0 0 auto;font-size:16px}.equipment-list-copy{display:flex;flex-direction:column;min-width:0}.equipment-list-name{font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.equipment-card-heading{display:grid;grid-template-columns:auto 1fr;gap:16px;align-items:center;margin:14px 0 18px}.equipment-card-icon{width:66px;height:66px;font-size:27px}.equipment-card-title,.equipment-card-original,.equipment-card-summary{display:block}.equipment-card-title{font:700 25px/1.15 Georgia,serif;color:var(--theme-heading)}.equipment-card-original{margin-top:2px;color:rgba(var(--theme-accent-rgb),.7);font-size:13px}.equipment-card-summary{max-width:720px;margin-top:8px;color:rgba(var(--theme-text-rgb),.72);line-height:1.52}
