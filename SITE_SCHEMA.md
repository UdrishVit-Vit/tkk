...
| --- | --- | --- |
| Уход | `is-turn-out` | 150 мс с разгоном: карточка уходит на 34% ширины в сторону жеста и гаснет до .2 |
| Перенос | `is-turn-jump` | без перехода: содержимое подменяется, карточка оказывается на 26% ширины с другой стороны |
| Приход | `is-turn-in` | 320 мс с торможением: карточка возвращается на место и проявляется |
